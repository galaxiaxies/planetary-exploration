var distance = Math.random * 1000000000;
var shipSpeed = 605000;
...
function logMake(message) {
	"use strict";
	(function () {
		$('#log').append('<span class=' + date.season + '>' + date.season + ' ' + date.day + ', ' + date.year + '</span>' + ': ' + message + '.');
	}());
}
	...
	planet: {
		distance: {
			update: function () {
				"use strict";
				(function () {
					$('#distance').html('ship progress:' + format(ship.progress) + ' km/' + format(distance) + ' km');
					console.log(format(resources.money));
					console.log('money update');
				}());
			}
...
var station = {
	buttons: {
		...
		ship: {
			...
			active: true,
			cost: true,
			price: round((distance / shipSpeed) * 10, 2),
...
var gameStart = {
	...
	start: function () {
		...
		(function () {
			...
			// $('send-ship').click(events.sendShip);
		}());
